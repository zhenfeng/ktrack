...
/** "SimpleMotionTracker"
  *
  * FOR DETECTING & TRACKING SMALL BLOBS IN THE PRESENCE OF CAMERA MOTION/JERK
  *
  */
class SimpleMotionTracker : public TrackingProblem
{
  virtual void evaluateLikelihood(const vector<Mat> &particles_in,
                                  vector<double> &likelihood)
  {
    ...
    int N = Nparticles();
    for( int k = 0; k < N; k++ ) {
      double fx = particles_in[k].at<double>(0);
      double fy = particles_in[k].at<double>(1);
      int x = (int) fx;
      int y = (int) fy;
      likelihood[k]  = 1e-2 + std::max(0.0, dIdt_grey.at<unsigned char>(y,x) * 1.0 - I_hilo.at<unsigned char>(y,x) * 1.0 ) ;
      likelihood[k] *= 1.0 /( 5.0 + std::fabs( fx - particles[k].at<double>(0) ) );
      likelihood[k] *= 1.0 /( 5.0 + std::fabs( fy - particles[k].at<double>(1) ) );
    }
  }
                                   ...
                                   void *extra_data)
  {
    ...
    cv::GaussianBlur(dIdt_grey,I_hilo,Size(11,11),5.0,5.0);
    bool printF2F = true;
    if( printF2F ) {
...
int main( int argc, char* argv [] )
{
  ...
  bool bWriteImages = (argc > 3); // get it from arg!
